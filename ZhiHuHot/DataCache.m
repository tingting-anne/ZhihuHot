...
- (id)initWithNamespace:(NSString *)ns {
    if ((self = [super init])) {
        NSString *fullNamespace = [@"com.hackemist.DataCache." stringByAppendingString:ns];
        
        // Create IO serial queue
        _ioQueue = dispatch_queue_create("com.hackemist.DataCache", DISPATCH_QUEUE_SERIAL);
...
- (void)storeData:(NSData *)data forKey:(NSString *)key {
    [self storeData:data recalculateFromData:YES forKey:key toDisk:YES];
}

- (void)storeData:(NSData *)data forKey:(NSString *)key toDisk:(BOOL)toDisk {
    [self storeData:data recalculateFromData:YES forKey:key toDisk:toDisk];
}
...
- (void)queryDiskCacheForKey:(NSString *)key done:(DataCacheQueryCompletedBlock)doneBlock {
    ...
    NSData *data = [self dataFromMemoryCacheForKey:key];
    if (data) {
        NSLog(@"%s load from memory", __FUNCTION__);
